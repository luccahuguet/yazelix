...
def merge_toml_files [yazelix_content: string, user_content: string, file_name: string] {
    ...
    let yazelix_toml = if ($yazelix_content | str trim | is-empty) {
        ...
        try {
            ...
            print $"⚠️  Warning: Could not parse Yazelix TOML for ($file_name), using as raw text"
            {}
        }
    }
    ...
    let user_toml = if ($user_content | str trim | is-empty) {
        ...
        try {
            ...
            print $"⚠️  Warning: Could not parse user TOML for ($file_name), using as raw text"
            {}
        }
    }
    ...
    let toml_content = if ($merged_toml | is-empty) {
        ...
        try {
            ...
            print $"⚠️  Warning: Could not generate TOML for ($file_name), falling back to concatenation"
            [
                $"# === YAZELIX DEFAULTS ===",
                $yazelix_content,
                "",
