...
def test_managed_nushell_config_loads_generated_yzx_extern_bridge [] {
    ...
    let result = (try {
        ...
        let output = (with-env {
            ...
            sync_generated_yzx_extern_bridge $repo_root $state_dir | ignore
            ^nu -c $"source \"($repo_root | path join "nushell" "config" "config.nu")\"; scope commands | where name in [\"yzx update\", \"yzx update upstream\", \"yzx update home_manager\", \"yzx update nix\", \"yzx update runtime\", \"yzx uninstall\"] | sort-by name | to json -r" | complete
        })
