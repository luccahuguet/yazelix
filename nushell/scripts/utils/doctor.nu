...
export def build_zellij_plugin_health_results [plugin_state: record, sidebar_enabled: bool] {
    ...
    if not ($plugin_state.permissions_granted? | default false) {
        $results = ($results | append {
            ...
            details: "Grant permissions for both Zellij plugins: focus the top zjstatus bar and press `y`, and also answer yes to the Yazelix orchestrator permission popup. If the popup never appears, run `yzx repair zellij-permissions` and restart Yazelix. Yazelix workspace bindings like `Alt+m`, `Alt+y`, `Ctrl+y`, `Alt+r`, `Alt+[`, and `Alt+]` depend on the orchestrator."
            fix_available: false
        })
    } else {
        $results = ($results | append {
