...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    trail = applyTrailLayer(trail, saturate(duskBase, 1.35), trailGlowMask(sdfTrail, mod + 0.010, 0.035));
    trail = applyTrailLayer(trail, saturate(duskEdge, 1.45), trailEdgeMask(sdfTrail, mod, 0.006));
    trail = mix(trail, saturate(duskBase, 1.4), trailCoreMask(sdfTrail, mod));

    trail = applyTrailLayer(trail, saturate(duskEdge, 1.5), cursorGlowMask(sdfCurrentCursor, .002, 0.004));
    trail = applyTrailLayer(trail, saturate(duskBase, 1.45), cursorEdgeMask(sdfCurrentCursor, .002, 0.004));
