...
mod tests {
    ...
    fn renders_inline_code_with_plain_markers() {
        let output = render_tutor_markdown_with_color(
            "Run `yzn tutor begin` then press `Alt Shift M`.",
            false,
        )
        .unwrap();
        assert_eq!(output, "Run `yzn tutor begin` then press `Alt Shift M`.\n");
    }

    #[test]
    fn renders_inline_code_with_ansi_when_color_is_enabled() {
        let output = render_tutor_markdown_with_color("Run `yzn tutor begin`.", true).unwrap();
    ...
    fn indents_list_item_hard_break_continuations() {
        let output = render_tutor_markdown_with_color(
            "1. `yzn tutor workspace` Workspace roots  \n   Practice the current tab root.\n",
            false,
        )
        .unwrap();
        assert_eq!(
