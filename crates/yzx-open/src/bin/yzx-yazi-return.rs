...
fn run(config: &Config, role: Option<&str>) -> Result<()> {
    if role == Some("workspace-popup") {
        popup_pipe(config, "close", "yazi")?;
    } else {
        orchestrator_pipe(config, "focus_editor", "")?;
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn popup_closes_while_sidebar_focuses_the_editor() {
        ...
        assert_eq!(
            ...
            "action pipe --plugin yzpp --name close -- yazi\n\
