...
use support::{
    ...
    set_test_nu, successful_output, successful_stdout, write_nu_executable,
};

fn main() {
    let args = env::args().collect::<Vec<_>>();
    let [_, yzx, closure, expected_variant, nu] = args.as_slice() else {
        panic!(
            "usage: no-helix-contracts-check <yzx-package> <closure-paths> <expected-variant> <nu>"
        );
    };
    ...
    write_nu_executable(
        &external,
        "def --wrapped main [...args: string] {\n    ($args | str join \"\\n\") + \"\\n\" | save --raw $env.YZX_EXTERNAL_EDITOR_OUT\n}\n",
    );
