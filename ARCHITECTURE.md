...
for native Nu, Starship, and managed Yazi init files, lists read-only packaged
...
home marker, and keeps the native bottom Zellij `status-bar` for key hints.
...
preview/session environment, appends optional managed user `yazi/init.lua`,
overlays managed user `plugins/*.yazi` directories when that init exists, and
