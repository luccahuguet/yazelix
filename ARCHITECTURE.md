...
global appearance mode. Low-level `force-theme` and `[colors]` entries remain
manual native Mars TOML. `yzn` still owns the launch command and runtime
environment.
...
| C2 | Mars uses packaged visual config unless a user native Mars config exists; the config UI exposes the Mars-owned dark/light `mars.appearance.preset`, while low-level `force-theme`, `[colors]`, and cursor TOML remain native/manual | `mars.toml`, `flake.nix`, `crates/yzn-config/` | `checks/yzn-contracts.rs` validates packaged config and launcher selection; `crates/yzn-config` tests validate Mars appearance routing and manual native TOML preservation | Visual correctness remains manual dogfooding |
