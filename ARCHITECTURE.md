...
Mars window, font, cursor, bell, quit, color, and theme behavior used by `yzn`.
`yzn config` exposes selected Mars-native fields, including `force-theme` and
core color fields, through `~/.config/yazelix-next/mars/config.toml`; root
`config.toml` does not own a global appearance mode. `yzn` still owns the
launch command and runtime environment.
...
| C2 | Mars uses packaged visual config unless a user native Mars config exists; Mars-native theme and color edits live in `mars/config.toml`, while low-level cursor TOML remains native/manual until Mars owns semantic presets | `mars.toml`, `flake.nix`, `crates/yzn-config/` | `checks/yzn-contracts.rs` validates packaged config and launcher selection; `crates/yzn-config` tests validate Mars theme/color routing and manual cursor TOML preservation | Visual correctness remains manual dogfooding |
