...
core colors, through `~/.config/yazelix-next/mars/config.toml`; root
...
| C2 | Mars uses packaged visual config unless a user native Mars config exists; Mars-native theme and color edits live in `mars/config.toml` | `mars.toml`, `flake.nix`, `crates/yzn-config/` | `checks/yzn-contracts.rs` validates packaged config and launcher selection; `crates/yzn-config` tests validate Mars theme/color routing | Visual correctness remains manual dogfooding |
