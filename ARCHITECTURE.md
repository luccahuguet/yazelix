...
replace the Mars config with `~/.config/yazelix-next/mars/config.toml`; `yzn`
still owns the launch command and runtime environment.
...
| C2 | Mars uses the packaged dark or light visual config from `appearance.mode` unless a user native Mars config exists | `mars.toml`, `flake.nix`, `runtime/yzn/` | `checks/yzn-contracts.rs` validates packaged dark/light config availability and launcher selection | Visual correctness remains manual dogfooding |
