...
The upgrade UX depends on durable, machine-readable release notes rather than ad hoc markdown edits or commit archaeology. Without an explicit contract and validator lane, it is too easy to change config surfaces or bump versions without recording what users need to know.
...
- optional major-series summaries in `docs/upgrade_notes.toml` for README fallback projections
...
- mutating issue-tracker or GitHub state from CI
