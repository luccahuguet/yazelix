...
- Statement: The integrated Yazelix status bar should not compile the `yazelix_zellij_bar` child crate into `yazelix_core` solely to render small KDL snippets. The child package remains the owner of standalone artifacts and runnable widget commands; the main repo owns the session-specific adapter and path selection
