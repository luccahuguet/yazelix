...
User-defined popup bindings live in `zellij.custom_popups`, not the static action registry. The default `zenith` popup is a custom popup entry with `keybindings = ["Alt Shift I"]` and `keep_alive = true`.
