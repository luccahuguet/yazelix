...
- In the current implementation, that boundary is the Rust `runtime_contract` evaluation library called by launch, startup, and doctor owners.
...
- Nushell still owns:
  ...
  - `yzx doctor --fix` actions and install-audit checks
...
- Doctor-only install-artifact audits, Helix runtime diagnostics, distribution-tier reporting, and session-local/plugin-local health remain outside this helper.
...
- redefining the remaining delete-first control-plane ownership already covered
  by the runtime and bridge contracts
...
- deciding the final `Yazelix Core` product boundary in this contract
