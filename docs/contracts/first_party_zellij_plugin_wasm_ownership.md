...
- Statement: First-party Zellij plugin child packages consumed by Yazelix must
  ...
  cargo-auditable wrapper, disabled Cargo build hook, preservation of the
  Nix-provided Rust toolchain across `preBuild`, wasm-target Rustc environment
  pinning, serialized Cargo build, and non-empty wasm install check
