...
| [`mars`](https://github.com/luccahuguet/mars) | Rio | Dogfood a first-party Rust terminal path with Yazelix-controlled Nix packaging, package metadata, profiles, protocol coverage, notifications, Kitty graphics, and cursor shaders | mars package profiles, desktop wrapper, metadata passthru, BELL/terminal notification behavior, Rio trail defaults, `yazelix-cursors` shader support, Kitty graphics support, and shader ABI | Experimental active fork | Monthly while dogfooding, and before mars runtime release evidence | Graduate to stable only after release-profile validation and real user value; upstream or delete local terminal deltas that become Rio-owned | [`mars_package_boundary.md`](./mars_package_boundary.md), [`mars_fast_dogfooding.md`](../mars_fast_dogfooding.md) |
...
| `mars` | `raphamorim/rio` | Yazelix maintainer | Monthly while mars is actively dogfooded, and before mars release-gate decisions | Terminal-repo review record, plus `docs/yazelix/fork_feature_verification.md` for release evidence |
...
| `mars` | Structured experimental active-fork delta block present in the child README | Keep current during monthly mars reviews |
