...
| --- | --- | --- | --- | --- | --- | --- | --- |
| [`mars`](https://github.com/luccahuguet/mars) | Rio | Originally gave Yazelix a controlled Rust terminal path for stack compatibility, Kitty-protocol work, native cursor behavior, and agent-driven development workflows; Kitty is now the packaged default terminal (operator directive 2026-07-11), so Mars is retained only for legacy `yzx enter` session-detection compatibility and a dormant generated-config materializer | Mars package identity, generated config contract, Yazelix cursor integration, launch/runtime metadata, and selected renderer/protocol fixes over upstream Rio, scoped to the legacy path only | Active fork | Monthly and whenever terminal launch, graphics, cursor, or agent workflow behavior changes | Upstream reusable pieces where practical; Mars is no longer needed for packaged-default terminal co-evolution now that Kitty is packaged, but is kept while the legacy `yzx enter` path still benefits from it | [`v15_trimmed_runtime_contract.md`](./v15_trimmed_runtime_contract.md), [`terminal_launch_contract.md`](./terminal_launch_contract.md) |
...
| [`yazelix-zellij`](https://github.com/luccahuguet/yazelix-zellij) | Zellij | Restore Yazi image previews through Kitty graphics passthrough in the Kitty runtime path | Temporary KGP preview branch selected by Yazelix package outputs | Temporary fork | Monthly and whenever upstream Zellij changes Kitty graphics behavior | Drop and archive once upstream Zellij supports the required Kitty graphics path directly enough for Yazelix to return to upstream packages | [`v15_trimmed_runtime_contract.md`](./v15_trimmed_runtime_contract.md) |
