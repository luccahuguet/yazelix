...
| [`yazelix-zellij`](https://github.com/luccahuguet/yazelix-zellij) | Zellij | Restore Yazi image previews through Kitty graphics passthrough in Mars, Ghostty, and other Kitty-passthrough runtime variants | Temporary KGP preview branch selected by Yazelix package outputs | Temporary fork | Monthly and whenever upstream Zellij changes Kitty graphics behavior | Drop and archive once upstream Zellij supports the required Kitty graphics path directly enough for Yazelix to return to upstream packages | [`v15_trimmed_runtime_contract.md`](./v15_trimmed_runtime_contract.md) |
