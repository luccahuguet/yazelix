...
- The packaged runtime ships one terminal variant at a time: Ghostty by default, with explicit `yzxterm`, `yazelix_wezterm`, Linux-only `yazelix_ratty`, and `yazelix_ghostty` variants for users who want a specific first-party path.
