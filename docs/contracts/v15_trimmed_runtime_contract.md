...
- The packaged runtime has one terminal variant at a time: Mars by default, with explicit `yazelix_mars`, `yazelix_ghostty`, `yazelix_rio`, `yazelix_wezterm`, `yazelix_kitty`, and Linux-only `yazelix_foot`/`yazelix_ratty` variants for users who want a specific path. Home Manager selects the same one-terminal model through `programs.yazelix.terminal`.
- The Mars and Ghostty paths use Yazelix-pinned Zellij and Yazi forks so Yazi image previews can use Kitty graphics through Zellij.
...
- Kitty remains a supported packaged alternative through the selected `kitty` runtime variant.
