...
- The packaged runtime has one terminal variant at a time: Ghostty by default, with explicit `yazelix_rio`, `yazelix_wezterm`, `yazelix_kitty`, Linux-only `yazelix_foot`/`yazelix_ratty`, and `yazelix_ghostty` variants for users who want a specific path. Home Manager selects the same one-terminal model through `programs.yazelix.terminal`.
