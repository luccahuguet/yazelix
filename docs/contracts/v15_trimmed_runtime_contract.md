...
- The packaged runtime has one primary terminal variant at a time: Ghostty by default, with explicit `yzxterm`, `yazelix_wezterm`, `yazelix_kitty`, Linux-only `yazelix_ratty`, and `yazelix_ghostty` variants for users who want a specific first-party path. Home Manager can additionally install terminal emulator packages through `programs.yazelix.extra_terminal_variants` without adding duplicate `yzx` wrappers.
...
- Kitty remains a supported alternative when the user provides that binary on `PATH`, but Yazelix can also package it as the selected `kitty` runtime variant.
