...
| `appearance_mode = "dark"`, `"light"`, or `"auto"` | `dark` | Implemented in Home Manager: declarative value for `appearance.mode`, passed to mars launch/session surfaces and consumed by generated config materialization | Ghostty and WezTerm use native static/auto theme selection; Rio and Foot use generated static palettes; mars uses child-owned dark/light themes and preserves child adaptive themes for `auto`; Zellij and Yazi change only their implicit default themes | Keep implemented |
...
| `mars_emoji_font = "noto"`, `"twitter"`, or `"serenityos"` | `noto` | Implemented in Home Manager: declarative value for `terminal.emoji_style`, selecting a child-owned Mars Terminal emoji fallback preset for activation, Linux desktop launches, and new shell sessions | Main Yazelix passes `MARS_EMOJI_FONT` with `MARS_EMOJI_FONT_SOURCE=home-manager` for managed materialization and desktop launch surfaces; direct child-wrapper launches can still read the session `MARS_EMOJI_FONT`, while unmarked main materialization reads `terminal.emoji_style`; mars owns the bundled font paths, font family names, and Rio font fallback semantics | Keep implemented |
