...
| `terminal = "ghostty"`, `"kitty"`, `"rio"`, `"yzxterm"`, `"wezterm"`, or Linux-only `"ratty"` | `ghostty` | Implemented: selects one packaged terminal instead of bundling every terminal or selecting terminals through config | Generated terminal config follows the selected terminal; Yazelix Terminal additionally reuses the child package profile config, injects `terminal.transparency`, and keeps default Rio trail cursor without packaged `custom-shader` | Keep implemented |
