...
| `runtime_variant = "ghostty"`, `"kitty"`, `"yzxterm"`, `"wezterm"`, or Linux-only `"ratty"` | `ghostty` | Implemented: selects one packaged terminal variant instead of bundling every terminal as the active runtime terminal | Generated terminal config follows the selected runtime variant; Yazelix Terminal additionally reuses the child package profile config, injects `terminal.transparency`, and keeps default Rio trail cursor without packaged `custom-shader` | Keep implemented |
| `yzxterm_profile = "full"`, `"baseline"`, or `"shaders"` | `full` | Implemented in Home Manager: selects the generated Yazelix Terminal profile for activation and Linux desktop launches | `full` keeps Rio trail cursor without custom shaders, `baseline` disables effects, and `shaders` passes `YAZELIX_TERMINAL_PROFILE=shaders` directly into activation plus the desktop entry so app-launcher environments do not decide the profile | Keep implemented |
