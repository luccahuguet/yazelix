...
| `runtime_variant = "ghostty"`, `"wezterm"`, or Linux-only `"ratty"` | `ghostty` | Implemented: selects one packaged terminal variant instead of bundling every terminal as the active runtime terminal | Generated terminal config follows the selected runtime variant | Keep implemented |
