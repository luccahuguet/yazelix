...
| `components.yazi_assets` / `yazelix-yazi-assets` | enabled | Not implemented yet: reusable flavors, reusable plugins, and the bundled Starship Yazi config are linked from the child asset pack | Invalid until the Yazi writer can render a first-party-only profile that does not reference child-provided flavors, `auto-layout.yazi`, `git.yazi`, `lazygit.yazi`, `starship.yazi`, or `yazelix_starship.toml` | Defer behind reduced profile |
...
The `yazelix-yazi-assets` asset pack is the next plausible future component toggle because it is a real child repository with package contents that can be omitted. It is not accepted as a direct boolean yet. The current default Yazi runtime links child flavors, `auto-layout.yazi`, `git.yazi`, `lazygit.yazi`, `starship.yazi`, and `yazelix_starship.toml`; generated keymaps and sidebar behavior can still call those plugins. A supported disabled state first needs a documented first-party-only profile that keeps Yazelix-owned plugins coherent, removes child asset references from generated config, records the disabled component in the manifest, and lets doctor report invalid references instead of silently degrading.
