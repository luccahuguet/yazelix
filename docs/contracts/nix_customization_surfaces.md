...
| `appearance_mode = "dark"`, `"light"`, or `"auto"` | inherited | Implemented in Home Manager: declarative value for `appearance.mode` when `manage_config = true`; otherwise the absent field follows the packaged semantic default | Zellij and Yazi change their implicit default themes where supported; Mars owns its native appearance | Keep implemented |
| `components.cursors` | enabled | Implemented partial package omission: cursor shader assets and `yazelix_cursors_default.toml` are removed from the runtime tree; cursor registry code remains linked into `yazelix_core` until crate-level feature gates exist | Standalone cursor bootstrap and UI fields are skipped; Mars keeps its independent native cursor config | Keep implemented |
