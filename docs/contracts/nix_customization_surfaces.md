...
| `runtime_variant = "ghostty"`, `"kitty"`, `"yzxterm"`, `"wezterm"`, or Linux-only `"ratty"` | `ghostty` | Implemented: selects one packaged terminal variant instead of bundling every terminal as the active runtime terminal | Generated terminal config follows the selected runtime variant; Yazelix Terminal additionally reuses the child package profile config, injects `terminal.transparency`, keeps default Rio trail cursor without packaged `custom-shader`, and honors `YAZELIX_TERMINAL_PROFILE=shaders` by injecting generated Yazelix cursor shader paths | Keep implemented |
