...
| `runtime_variant = "ghostty"`, `"yzxterm"`, `"wezterm"`, or Linux-only `"ratty"` | `ghostty` | Implemented: selects one packaged terminal variant instead of bundling every terminal as the active runtime terminal | Generated terminal config follows the selected runtime variant; Yazelix Terminal additionally reuses the child package config and injects `terminal.transparency` | Keep implemented |
