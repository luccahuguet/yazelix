...
| `~/.config/yazelix/mars/config.toml` generic TOML rows | native file status | Mars reads the complete native file on the next Mars window; Home Manager-owned files remain read-only |
