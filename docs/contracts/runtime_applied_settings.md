...
| `~/.config/yazelix/mars/config.toml` generic TOML rows | native file status | Mars overlays sparse user values on the live packaged base in the next Mars window; Home Manager-owned files remain read-only |
