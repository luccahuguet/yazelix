...
| `terminal.config_mode`, `terminal.emoji_style`, `terminal.transparency` | `shell_terminal_restart` | Terminal config changes apply to newly launched terminal processes, or to Home Manager activation when Home Manager owns the source |
...
| `zellij.widget_frame`, `zellij.widget_separator` | Yazelix passes widget chrome settings to the child runtime KDL template renderer | Changing frames or separators changes loaded zjstatus format strings, pipe-widget formats, and command-widget arguments |
