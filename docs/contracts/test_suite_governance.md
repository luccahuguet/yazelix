...
| Zellij layouts, first-party plugin contracts, or workspace assets | Focused Rust/plugin tests for touched behavior | `yzx_repo_validator validate-workspace-session-contract`; `yzx_repo_validator validate-pane-orchestrator-sync` after pane-orchestrator source changes; `yzx_repo_validator validate-yzpp-sync` after yzpp artifact changes |
