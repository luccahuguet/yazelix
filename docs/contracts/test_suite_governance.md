...
| Zellij layouts, first-party plugin contracts, or workspace assets | Focused Rust/plugin tests for touched behavior | `yzx_repo_validator validate-workspace-session-contract`; `nix build .#yazelix` after first-party plugin package or runtime artifact changes |
