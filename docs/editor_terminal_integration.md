...
If you want to keep the editor's current shell instead of switching into the shell configured in `config.toml`, use:
...
- `yzx env` then loads the Yazelix environment and switches into the shell configured in `config.toml`.
