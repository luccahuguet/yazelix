...
- The packaged runtime ships one terminal variant at a time: WezTerm by default on Linux, Ghostty by default on macOS, and explicit `yazelix_wezterm` / `yazelix_ghostty` variants for users who want the other first-party path.
