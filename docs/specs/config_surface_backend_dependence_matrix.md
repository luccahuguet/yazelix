...
| Terminal visual styling | `terminal.transparency`, plus Ghostty cursor settings in `user_configs/yazelix_cursors.toml` | integration/UI policy | no | keep | keep only if Yazelix still owns terminal config generation; otherwise move outward |
