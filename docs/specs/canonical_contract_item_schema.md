...
Yazelix should use an indexed contract item as the durable unit of spec and
...
Current Yazelix specs and tests are partly traceable, but the traceability is
...
- the canonical contract item unit for Yazelix specs
...
- migration rules for existing specs and tests
...
- `quarantine`: temporarily retained legacy behavior or traceability debt that
  must not expand and must carry an exit bead
...
For `live`, `planning`, `deprecated`, and `quarantine` items, the spec should
record these fields:
...
- `unverified`: intentionally unverified for now, with a reason and exit bead

Rules:

- `live` items should normally have at least one `automated`, `validator`, or
  `manual` entry
- `unverified` is allowed only when the reason is explicit and an owning bead
...
- regression-only tests are allowed, but they must still name the bug or
  invariant explicitly and should point to the related spec or bead when that
  mapping debt is temporary
- a broad policy-only reference such as only defending
  `docs/specs/test_suite_governance.md` is not sufficient for default-lane test
  traceability
...
- new specs that define live normative behavior should use contract items
- touched specs should add IDs only for the live normative items relevant to the
  current change
...
- when a spec is split or merged, surviving item IDs should stay stable where
...
Before code, aliases, wrappers, bridges, specs, or tests are removed, the work
must name:
...
- `quarantine` items must always name the bead that will either delete,
  remap, or retire them

## Non-goals

- indexing every line of every spec
- rewriting every historical spec in one pass
...
1. A maintainer can write one live contract item with a stable ID, named owner,
   and explicit verification without turning the whole spec into a database dump
...
- manual verification:
  ...
  - use `yazelix-rdn7.7` to pilot the schema on one subsystem before broad
    validator rollout
