...
| Startup bootstrap exports explicit runtime, config, state, and logs roots before entering the live shell and keeps shellhook setup measurable | `docs/specs/runtime_root_contract.md`; `docs/specs/v15_trimmed_runtime_contract.md`; `docs/specs/startup_profile_scenarios.md` | POSIX `runtime_env.sh`; POSIX `start_yazelix.sh`; Nu `setup/environment.nu`; Nu `startup_profile.nu` | `test_startup_bootstrap_runtime_env_exports_state_and_logs_dirs` in `nushell/scripts/dev/test_yzx_workspace_commands.nu`; `test_startup_profile_harness_records_real_startup_boundaries`; `validate_installed_runtime_contract.nu` | same split |
...
- `yazelix-zlt2`
  ...
  - verification that must still pass: `test_yzx_generated_configs.nu`, detached-launch tests, `validate_installed_runtime_contract.nu`
