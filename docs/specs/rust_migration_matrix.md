...
| Doctor and install report bridges | public report owner in `yzx_control`; private Nu helpers in `doctor_fix.nu` and `install_ownership.nu` | The public doctor owner and the shared report bridge are gone. Rust now owns report aggregation, summary/rendering, JSON, and live Zellij plugin-health checks. The surviving Nu work is a private repair helper plus focused install-ownership request shaping. | Keep the private fix helper and install-ownership helper narrow. Do not recreate a shared doctor-report bridge or a public Nu doctor owner. | Landed under `yazelix-5ewl.3` |
...
- do not delete it by copying `build_runtime_materialization_context` into
  `start_yazelix_inner.nu`, `doctor_fix.nu`, and `install_ownership.nu`
