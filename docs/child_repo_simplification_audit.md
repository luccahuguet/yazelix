...
- Rust budget ceiling: `73,542` raw tracked Rust lines across `140` files
...
| 2 | Finish yzpp cleanup tail | Delete-only / thin adapter | `transient-pane-facts.compute`, `transient_pane_facts.rs`, popup/session facts tests | Small LOC change, high ownership clarity; closes stale popup terminology after yzpp extraction | Do `yazelix-g7bs.2` after the build-wrapper fix |
...
2. `yazelix-g7bs.2`: top ownership-clarity cleanup. Expected impact is small but it prevents the main repo from implying that popup lifecycle still belongs to the pane orchestrator after `yzpp`.
