...
- one packaged terminal: Kitty in `#yazelix` and `#yazelix_kitty`
...
shells/posix/yazelix_runtime_size_report.sh --build .#yazelix_kitty --top 40 --direct-top 60
```
...
Measurements below are local NAR/closure measurements from June 2, 2026, before Mars became the default terminal (Kitty superseded Mars as the packaged default terminal on 2026-07-11). They are not exact Cachix billed bytes because Cachix checks the upstream NixOS cache first and uploads compressed paths.
...
The Linux runtime registry imports `nixGL` when the platform is Linux and the flake input is present, with `enable32bits = false` and `enableIntelX86Extensions = false`. The Kitty package path may add a 64-bit-only Vulkan wrapper for the packaged terminal launcher.

Current launch behavior:

- Kitty launch commands may prepend a graphics wrapper
- terminals outside packaged Kitty keep their own graphics-wrapper policy and run Yazelix with `yzx enter`

The option surface should be explicit instead of hidden behind `runtime_tool_sources`: a future package option should choose a graphics wrapper source such as bundled, host, or none, with launch rendering and doctor diagnostics tested against Kitty. Default behavior should not change until desktop launch reliability is preserved.
...
| `yazelix_kitty` | measure before release | measure before release | Keep: explicit Kitty runtime path |

The previous workflow also listed non-Kitty terminal outputs, `yazelix_agent_tools`, and `yazelix_screen`; those are no longer explicit publish targets. Further storage relief should come from shrinking the default runtime closure, especially host-tool-manager references, and from Cachix retention policy.
