...
| --- | ---: | ---: | --- | --- |
| Product runtime source | 93 | 46,932 | canonical runtime and integration adapters | High: contains the largest user-facing seams |
| Product integration tests | 19 | 5,699 | canonical behavior tests | Medium: split by behavior family, do not delete broadly |
| Maintainer tooling and tests | 24 | 10,586 | canonical maintainer tooling | Medium: keep in repo, but keep splitting large validator files |
...
| --- | ---: | ---: | ---: | --- |
| `core_cli_and_public_surface` | 12 | 7,957 | 7,000 | Public command dispatch, command metadata, front-door rendering, and support/onboarding commands |
...
| `maintainer_tooling_and_validators` | 23 | 10,347 | 9,000 | In-repo validators, Beads/GitHub sync, release/update workflow, sweep/test runners, and repo maintenance commands |
...
| --- | ---: | --- | --- |
| `rust_core/yazelix_core/src/zellij_materialization.rs` | 2,740 | generated Zellij config materialization | Keep until keybinding ownership and layout-generation contracts settle |
| `rust_core/yazelix_core/src/config_ui.rs` | 2,532 | Yazelix config UI adapter | Continue thinning toward the private `yazelix_ratconfig` boundary while keeping Yazelix settings/apply/Home Manager adapters local |
| `rust_core/yazelix_core/src/bin/yzx_control.rs` | 1,711 | public control-plane command implementation | Split only if routing remains obvious |
| `rust_core/yazelix_core/src/bin/yzx_core.rs` | 1,411 | machine helper used by shell/bootstrap surfaces | Collapse only after callers have a stable replacement |
| `rust_core/yazelix_core/tests/yzx_core_config_normalize.rs` | 1,406 | config/materialization integration tests | Split by behavior family; do not delete without replacement coverage |
| `rust_core/yazelix_core/src/doctor_commands.rs` | 1,403 | doctor orchestration | Split report rendering from fix orchestration only after doctor behavior stabilizes |
...
| `rust_core/yazelix_core/src/install_ownership_report.rs` | 1,124 | install ownership diagnostics | Prune only after supported recovery paths are narrower |
...
| `rust_core/yazelix_maintainer/src/repo_update_workflow.rs` | 1,091 | maintainer update workflow | Keep in repo as mutating release/update workflow; split by runtime-pin sync, asset refresh, activation, and canary materialization |
| `rust_core/yazelix_core/src/public_command_surface.rs` | 1,058 | public command registry | Keep central registry; future action registry may absorb part of this |
...
| `rust_core/yazelix_core/src/front_door_render.rs` | 988 | public help/menu rendering | Keep readable; avoid new command-surface trivia tests |
| `rust_core/yazelix_core/src/config_normalize.rs` | 970 | config behavior normalizer | Split only around durable setting families |
| `rust_core/yazelix_maintainer/src/repo_contract_validation/config_surface.rs` | 933 | config-surface contract validator | Split only by clear contract families |
...
| `rust_core/yazelix_core/src/zellij_render_plan.rs` | 842 | typed Zellij render plan | Keep as source of truth for materialization fingerprints |
...
`config_ui.rs` keeps Yazelix settings, JSONC patching, Home Manager/read-only ownership, cursor config, and runtime apply policy. The private `yazelix_ratconfig` boundary owns reusable model/editor/render helpers including row filtering/search and row/detail rendering.
...
4. Continue splitting `config_ui.rs` behind the private `yazelix_ratconfig` boundary before considering public extraction
...
| `65k` | Achieved by status-provider/bar and pane-orchestrator extraction work | Treat future growth above this as budget debt unless product behavior requires it |
| `60k` | Requires roughly `3.2k` more raw budget-line reduction from `zellij_commands.rs`, `launch_commands.rs`, `config_ui.rs`, maintainer validators, or more public extraction | Next main-repo hard-target gate |
