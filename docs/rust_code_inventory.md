...
| --- | ---: | ---: | --- | --- |
| Product runtime source | 95 | 51,250 | canonical and extension surfaces | High: contains the largest user-facing seams |
...
| Total | 163 | 72,663 | current budget ceiling | Reduce or extract before raising ceilings |
...
| `core_workspace_and_pane_integration` | 33 | 16,486 | 11,000 | Action registry, launch private adapters, Zellij/session/workspace command surface, pane-orchestrator client, status cache IO, widgets, agent usage refreshers |
...
| `rust_core/yazelix_core/src/zellij_commands/status/tests/agent_usage.rs` | 1,220 | Agent usage cache and refresh regressions; split further by provider before adding new cases |
