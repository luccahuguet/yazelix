...
`yzx reset config` replaces only `~/.config/yazelix/settings.jsonc`. Managed override files such as `helix/`, `zellij.kdl`, the `yazi/` managed home, terminal snippets, shell hooks, legacy TOML inputs, and unknown adjacent files are preserved and reported so you can move them aside manually when you want a completely clean configuration.
