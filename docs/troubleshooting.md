...
- **Configuration health** - `config.toml` validation and shell integration
...
**Yazelix now uses `config.toml` and the packaged `yazelix` runtime instead of the old `yazelix.nix` flow.**
