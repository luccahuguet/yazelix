...
| Doctor and install report shims | `doctor_report_bridge.nu` plus `doctor.nu` | smaller after the public `yzx status` Rust owner cut deleted `status_report.nu` and the old per-report shims | Rust already owns the structured findings. One shared doctor-report transport seam is enough; `doctor.nu` should only render and fix. | Keep one report-transport seam and keep only the human rendering helpers and fix actions that still add product value |
