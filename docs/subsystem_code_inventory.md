...
| Runtime materialization bridge | `core/materialization_orchestrator.nu` | `129` raw lines before the planned cut | Rust now owns the runtime materialization lifecycle, and `q0o9.1` found that the remaining shared Nu bridge mostly exists to build request JSON for Rust commands that should derive it themselves | Delete it in `q0o9.2` by moving env/request materialization entrypoints into Rust; keep only caller-local startup and doctor progress/error rendering |
