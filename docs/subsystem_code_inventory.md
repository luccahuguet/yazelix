...
| --- | --- | --- |
| Public CLI UX for intentionally Nu-owned commands | `yzx/edit.nu`, `yzx/import.nu`, `yzx/menu.nu`, `yzx/dev.nu` | The compatibility registry `core/yazelix.nu` is deleted. Root help, palette inventory, and extern metadata now come from Rust, and `yzx screen`, `yzx tutor`, `yzx whats_new`, `yzx home_manager`, `yzx config`, `yzx cwd`, `yzx reveal`, `yzx doctor`, `yzx why`, `yzx sponsor`, and `yzx keys` are already Rust-owned. The remaining Nu command bodies should stay in Nu only while they remain the honest `fzf`, editor, popup, or maintainer shell boundary |
...
| Human rendering and front-door UX | `setup/welcome.nu`, `utils/front_door_runtime.nu`, `yzx/menu.nu`, `yzx/edit.nu`, `yzx/import.nu` | The old renderer/data owners are gone. Rust now owns screen playback, Game of Life, tutor, and upgrade-summary shaping. The remaining Nu floor is mostly startup-shell presentation and process handoff |
| Runtime integration glue around live tools | `integrations/*.nu`, `zellij_wrappers/*.nu`, `utils/editor_launch_context.nu` | The duplicate full-config reads are gone from the popup/editor wrapper seam. What remains mostly adapts to Zellij, Yazi, and editor process behavior rather than modeling deterministic domain state |
