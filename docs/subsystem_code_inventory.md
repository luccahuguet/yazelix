...
| Doctor and install report shims | focused `install_ownership.nu` plus private `doctor_fix.nu` | much smaller after the public Rust `yzx doctor` cut deleted `doctor_report_bridge.nu`, `doctor.nu`, and `core/yzx_doctor.nu` | Rust now owns the public doctor report path, summary/rendering, JSON emission, and live Zellij plugin-health reporting. The surviving Nu work is a private fix helper plus a narrow install-ownership request helper. | Keep the private fix helper narrow and do not recreate a shared doctor-report bridge or a public Nu doctor owner |
...
| --- | --- | --- |
| Public CLI UX for intentionally Nu-owned commands | `core/yazelix.nu`, `core/yzx_workspace.nu`, `core/yzx_session.nu`, `yzx/edit.nu`, `yzx/import.nu`, `yzx/menu.nu`, `yzx/popup.nu`, `yzx/screen.nu`, `yzx/tutor.nu`, `yzx/whats_new.nu`, `yzx/desktop.nu` | Root help, palette inventory, and extern metadata now come from Rust. `yzx home_manager`, `yzx config`, `yzx doctor`, `yzx why`, `yzx sponsor`, and `yzx keys` are now Rust-owned too. The remaining Nu command bodies should stay in Nu unless Rust becomes the single public owner for a whole relevant family |
