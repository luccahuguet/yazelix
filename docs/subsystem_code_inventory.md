...
| --- | --- | --- |
| Public CLI UX for intentionally Nu-owned commands | `yzx/menu.nu`, `yzx/dev.nu` | The compatibility registry `core/yazelix.nu` is deleted. Root help, palette inventory, and extern metadata now come from Rust, and `yzx edit`, `yzx import`, `yzx screen`, `yzx tutor`, `yzx whats_new`, `yzx home_manager`, `yzx config`, `yzx warp`, `yzx reveal`, `yzx doctor`, `yzx why`, `yzx sponsor`, and `yzx keys` are already Rust-owned. The remaining Nu command bodies should stay in Nu only while they remain the honest `fzf`, popup, or maintainer shell boundary |
