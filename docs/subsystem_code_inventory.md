...
| --- | --- | --- |
| Public CLI UX for intentionally Nu-owned commands | `yzx/menu.nu` | The compatibility registry `core/yazelix.nu` is deleted. Root help, palette inventory, extern metadata, and `yzx dev` now come from Rust-owned control-plane routes. The remaining Nu command body should stay in Nu only while it remains the honest `fzf` menu boundary |
...
| Runtime integration glue around live tools | Rust `workspace_commands.rs`, Rust `zellij_commands.rs`, and the sidebar Yazi launcher under `zellij_wrappers/` | The Yazi/editor integration owners are now Rust-owned. The surviving Nu floor is limited to the sidebar Yazi launcher that still sits at a real shell/process boundary |
