...
Regular Yazelix users do not need to install it separately. The normal Yazelix package consumes the child crate for tab/status rendering and forwards the standalone package as `#yazelix_zellij_bar`.
...
Use the child README for Zellij layout examples and custom command-widget configuration. KDL is the customization surface; there is no generator binary or central `config.toml` for the bar.
