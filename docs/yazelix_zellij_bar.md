...
Regular Yazelix users do not need to install it separately. The normal Yazelix package consumes the child package for tab/status layout replacement data and forwards the standalone package as `#yazelix_zellij_bar`.
...
Use the child README for Zellij layout examples and custom command-widget configuration. KDL is the customization surface for standalone users. Integrated Yazelix calls `yazelix_zellij_bar_widget render-yazelix-runtime --json <request>` to get a placeholder replacement map for its generated layouts.
