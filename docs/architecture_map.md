...
| Distribution and host integration | How Yazelix is packaged, launched, and adapted into external owners such as Home Manager, shells, terminals, desktop integration, and profile-owned installs | `home_manager`, `packaging`, `shells`, `flake.nix` | The complete packaged runtime shape and explicit integration contracts |
...
| Workspace session orchestration | `yzx_repo_validator validate-workspace-session-contract`; `nix build .#yazelix` after first-party plugin package or runtime artifact changes |
...
For machine-readable inspection, use the Rust runtime ownership graph library
from validators or focused Rust tests.
