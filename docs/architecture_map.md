...
| --- | --- | --- | --- |
| Runtime control plane and command surface | Config parsing, runtime/bootstrap behavior, generated-state repair, and the `yzx` command surface | `rust_core/yazelix_core/src`, `nushell/scripts/core`, `nushell/scripts/setup`, `nushell/scripts/utils` | Runtime/config/state contracts plus the Rust-owned `yzx` command semantics |
