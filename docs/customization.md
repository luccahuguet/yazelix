...
- **Terminal Configurations**:
  - **Kitty**: The default `#yazelix` package, `#yazelix_kitty`, and `programs.yazelix.terminal = "kitty"` path use Kitty as the packaged default terminal. Kitty's native config is user-owned — Yazelix does not generate a Kitty config file.
  - **Ghostty**: The host-installed backup terminal (`launch_order=["kitty","ghostty"]`), started via `yzx enter`. It is the most tested mature host-terminal path, with a strong macOS story. Configure Ghostty to start `yzx enter`; run `yzx cursors ghostty setup` when you want Yazelix cursor shaders in a user-owned Ghostty config.
  - **Other terminals**: Rio, WezTerm, Foot, Ratty, Alacritty, and other capable emulators work by starting Yazelix with `yzx enter`; their native terminal config stays host-owned. Mars is `supported=false`/`packaged=false` and removed from the launch chain (operator directive 2026-07-11); it is retained only for legacy `yzx enter` session detection and a now-dormant generated-config materializer.
    ...
    - **Scrollback ownership**: Zellij owns pane history inside Yazelix and defaults to `scroll_buffer_size 5000` unless your managed/native Zellij config sets another active value. (Mars previously disabled terminal-emulator scrollback; that note no longer applies now that Kitty is the default terminal.)
    ...
    - **Transparency**: Configure `transparency = "none"`, `"low"`, `"medium"`, or `"high"` in `settings.jsonc`. Kitty's native config is user-owned, so this setting has no generated-config target for the default terminal; the dormant legacy Mars materializer still maps it into the generated Mars config for hosts still on that path.
    - **Config ownership switch** (legacy, Mars only, dormant): `terminal.config_mode = "yazelix"` kept using the Yazelix-managed Mars config; `"user"` loaded the host Mars config file instead and failed if it was missing. This switch does not apply to Kitty — Kitty's config is always user-owned.
    - **No manual copying required** (legacy Mars materializer, dormant) - previously generated automatically when launching Yazelix; Kitty and other host terminals use user-owned native configs
