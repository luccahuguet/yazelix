...
- **Terminal Configurations**:
  - **Bundled terminals** (yazelix-ghostty, Rio, WezTerm, Kitty, Foot, Ratty): Configs generated dynamically from your yazelix settings
    ...
    - **Appearance mode**: Set `appearance.mode = "dark"`, `"light"`, or `"auto"` in `settings.jsonc`. Dark is the default. Light switches Yazelix-owned generated terminal, Zellij, and Yazi defaults to light palettes where that terminal supports them, while preserving explicit `zellij.theme` and `yazi.theme` choices. Auto uses native appearance switching in Ghostty and WezTerm.
    - **Transparency**: Configure `transparency = "none"`, `"low"`, `"medium"`, or `"high"`; Yazelix maps that setting into Ghostty, Foot, Ratty, Kitty, Rio, and WezTerm generated configs
