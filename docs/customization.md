...
- **Terminal Configurations**:
  - **Bundled terminals** (yazelix-ghostty, mars, etc.): Configs generated dynamically from your yazelix settings
    ...
    - **Mars Terminal**: The experimental `#mars` or `programs.yazelix.terminal = "mars"` path reads the child package's bundled profile config, writes a generated Yazelix-owned config, preserves Rio trail cursor settings by default, and passes that generated config directory through `MARS_CONFIG`. Full and baseline profiles use the stable config under `~/.local/share/yazelix/configs/terminal_emulators/mars/config.toml`; the shader profile writes launch-scoped config and shader snapshots under `~/.local/share/yazelix/terminal_launches/<launch-id>/` so opening a new terminal window cannot rewrite GLSL files used by an existing mars process. Set `MARS_PROFILE=shaders` or `MARS_EFFECTS=shaders` to make the generated config include the active Yazelix cursor shader chain; set `baseline` or `none` to use the no-effects profile. Set `terminal.emoji_style = "twitter"` or `"serenityos"` to select one of the child-owned alternate emoji fallback presets for fresh mars windows; `noto` is the default. Declarative Home Manager setups set the same value with `programs.yazelix.manage_config = true` and `programs.yazelix.mars_emoji_font`.
    - **Rio**: The `#yazelix_rio` or `programs.yazelix.terminal = "rio"` path writes a generated Rio config under `~/.local/share/yazelix/configs/terminal_emulators/rio/config.toml` and launches Rio with `RIO_CONFIG_HOME` pointing at that generated directory. The generated config enables Rio's native `trail-cursor` effect, maps `terminal.transparency` to Rio window and cell opacity, and points Rio at packaged Nerd Font and emoji fallback directories. On Linux with transparency enabled and an X display available, Yazelix launches Rio through XWayland because upstream Rio currently ignores background opacity on COSMIC/Wayland.
    - **Scrollback ownership**: Packaged terminal configs disable terminal-emulator scrollback. Zellij owns pane history inside Yazelix and defaults to `scroll_buffer_size 5000` unless your managed/native Zellij config sets another active value.
    - **Appearance mode**: Set `appearance.mode = "dark"`, `"light"`, or `"auto"` in `settings.jsonc`. Dark is the default. Light switches Yazelix-owned generated terminal, Zellij, and Yazi defaults to light palettes where that terminal supports them, while preserving explicit `zellij.theme` and `yazi.theme` choices. Auto uses native appearance switching in Ghostty, WezTerm, and mars; mars owns its packaged dark/light themes in the terminal child package, and main Yazelix copies those themes into generated mars config roots.
    - **Transparency**: Configure `transparency = "none"`, `"low"`, `"medium"`, or `"high"`; Yazelix maps that setting into Ghostty, Foot, Ratty, Kitty, Rio, WezTerm, and Mars Terminal generated configs
