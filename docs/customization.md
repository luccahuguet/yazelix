...
- **Terminal Configurations**:
  - **Bundled terminals** (yazelix-ghostty, yazelix-terminal, etc.): Configs generated dynamically from your yazelix settings
    ...
    - **Yazelix Terminal**: The experimental `#yzxterm` or `programs.yazelix.terminal = "yzxterm"` path reads the child package's bundled profile config, writes a generated Yazelix-owned copy under `~/.local/share/yazelix/configs/terminal_emulators/yzxterm/config.toml`, preserves Rio trail cursor settings by default, and passes that directory through `YAZELIX_TERMINAL_CONFIG`. Set `YAZELIX_TERMINAL_PROFILE=shaders` or `YAZELIX_TERMINAL_EFFECTS=shaders` to make the generated config include the active Yazelix cursor shader chain; set `baseline` or `none` to use the no-effects profile.
