...
- **Terminal Configurations**:
  - **Bundled terminals** (yazelix-ghostty, yazelix-terminal, etc.): Configs generated dynamically from your yazelix settings
    ...
    - **Rio**: The `#yazelix_rio` or `programs.yazelix.terminal = "rio"` path uses the `yazelix-terminal` child package's Rio-compatible command, writes a generated Rio config under `~/.local/share/yazelix/configs/terminal_emulators/rio/config.toml`, and launches Rio with `RIO_CONFIG_HOME` pointing at that generated directory. The generated config enables Rio's native `trail-cursor` effect, leaves renderer backend selection to Rio's platform default, maps `terminal.transparency` to Rio window and cell opacity, and points Rio at packaged Nerd Font and emoji fallback directories.
