...
yzx profile --cold --clear-cache  # Profile after config change (toggles rounded_corners for re-evaluation)
...
# 3. Config change (~3-7s): After modifying yazelix.toml (triggers Nix re-evaluation)
```