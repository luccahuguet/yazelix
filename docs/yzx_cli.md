...
- `yzx tutor troubleshooting`: practice focus, popup, config, keybinding, and doctor recovery paths
...
- `yzx dev release VERSION [--dry-run] [--no-push] [--no-watch] [--dispatch-missing-workflows]`: Run the maintainer release transaction around `yzx dev bump`, auto-sync the GitHub/Beads contract when needed, run release validators, push `main`, watch `CI` plus `Publish Nix Cache`, then push the release tag only after required workflows pass
...
yzx_repo_validator validate-child-release-transaction  # Verify locked child commits are published and Darwin child package smoke contracts hold before landing a main lock update; targeted Nix package builds validate Cargo git output hashes
