...
- [mars](https://github.com/luccahuguet/mars) — Experimental Rio-derived Rust terminal path with strong Nix packaging, BELL notifications, Kitty graphics, protocol coverage, Rio trails, and `yazelix-cursors` shader support; exposed here as `#yzxterm` and `#runtime_yzxterm` for opt-in first-party terminal testing; maintainer fast dogfooding outputs are `#yzxterm_fast` and `#runtime_yzxterm_fast`
...
- **Terminal**: Ghostty is the default packaged terminal with Yazelix cursor trails and Yazi image previews, Yazelix Terminal is available through the experimental first-party Nix package path, WezTerm is available through the stable alternate package path, Foot and Ratty are available through Linux package paths, and Kitty is available through a packaged runtime variant or host `PATH`
...
- **Yazelix Terminal** (experimental first-party packaged path): Rio-derived Rust terminal with Nix-owned package profiles, generated transparency config, BELL notifications, protocol coverage, Kitty graphics, Rio trail cursor defaults, and opt-in `yazelix-cursors` shader support
