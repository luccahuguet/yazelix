...
- [yazelix-terminal](https://github.com/luccahuguet/mars) — Experimental Rio-derived Rust terminal path with BELL notifications, Kitty graphics, protocol coverage, Rio trails, and `yazelix-cursors` shader support; exposed here as `#yzxterm` and `#runtime_yzxterm` for opt-in first-party terminal testing; maintainer fast dogfooding outputs are `#yzxterm_fast` and `#runtime_yzxterm_fast`
