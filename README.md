...
- **Zellij customization**: Use the built-in `zellij` settings in `settings.jsonc` for Yazelix-owned Zellij knobs and keybindings, and use `~/.config/yazelix/zellij.kdl` for deeper non-keybinding Zellij settings (see [Zellij Configuration](./docs/zellij-configuration.md))
