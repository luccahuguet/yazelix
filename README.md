...
- **Status bar widgets**: Configure `[zellij].widget_tray` to order or hide `editor`, `shell`, `term`, `workspace`, `cursor`, usage, `cpu`, and `ram` widgets; the optional cursor widget renders as colored `█ name` from the launch-scoped Ghostty cursor fact
