...
- Kitty is the packaged default Yazelix terminal and Ghostty is the host-installed backup (operator directive 2026-07-11); the retained Mars renderer is no longer part of the supported launch chain. Terminal support metadata (ids, labels, capability hints, session detection) is owned by the `yazelix-terminal-support` child repo and consumed as the single source of truth. Other terminal emulators are host-owned entrypoints that should start Yazelix with `yzx enter`.
- Keep compile-heavy terminal commands (`cargo`, full `nix build`, or Home Manager switch) intentional. Prefer metadata evals and focused package/runtime checks first, then run the exact packaged-terminal gate required by the change.
