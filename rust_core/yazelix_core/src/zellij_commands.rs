...
fn print_zellij_open_editor_help() {
    println!("Open a directory in the managed editor via the pane orchestrator");
    println!();
    println!("Usage:");
    println!("  yzx_control zellij open-editor <path>");
    println!();
    println!("Returns JSON with status, workspace_root, editor_status, and sidebar_state.");
    println!("If editor_status is 'missing', the caller should open a new editor pane.");
}
...
pub fn run_zellij_open_editor(args: &[String]) -> Result<i32, CoreError> {
    ...
    let target_dir = resolve_target_dir(&target)?;
    let facts = crate::workspace_commands::compute_integration_facts_from_env()?;
    let editor_kind = facts.managed_editor_kind;

    if editor_kind.is_empty() {
        return Err(CoreError::classified(
            ErrorClass::Runtime,
            "no_managed_editor",
            "No managed editor detected. Set [editor] command in yazelix.toml.",
            "Configure a supported editor (helix or neovim) in yazelix.toml, then retry.",
            json!({}),
        ));
    }
    ...
    let status = result.get("status").and_then(|v| v.as_str()).unwrap_or("error");
    let editor_status = result
        .get("editor_status")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let sidebar_state = result.get("sidebar_state");
    ...
    match status {
        ...
        _ => {
            let reason = result
                ...
                .and_then(|v| v.as_str())
                .unwrap_or("unknown error");
            eprintln!("❌ Failed to open editor: {}", reason);
            Ok(1)
        }
    }
}
