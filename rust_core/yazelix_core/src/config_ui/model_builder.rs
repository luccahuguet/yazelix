...
pub fn build_config_ui_model(request: &ConfigUiRequest) -> Result<ConfigUiModel, CoreError> {
    ...
    let mars_rows = build_toml_document_fields(ConfigUiTomlDocumentSpec {
        ...
        default_toml: Some(packaged_mars_config.as_str()),
        ...
        apply_status: ConfigUiApplyStatus {
            ...
            detail: "Mars overlays these values on its packaged defaults in the next window."
                .to_string(),
            pending: true,
        },
    })
    .map_err(|message| {
        CoreError::classified(
            ...
            "invalid_mars_config",
            message,
            "Fix ~/.config/yazelix/mars/config.toml or remove it to use only packaged defaults.",
